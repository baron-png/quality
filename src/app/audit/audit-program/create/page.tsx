...
import { createAuditProgramWithAudits } from "@/api/auditService"; // <-- modular API call
...
export default function CreateAuditProgramPage() {
  ...
  const handleAuditInputChange = (e: ChangeEvent<{ name?: string; value: unknown }>) => {
  ...
  const handleMethodsChange = (e: any) => {
    setAuditInput((prev) => ({ ...prev, methods: e.target.value as string[] }));
  };

  const handleCriteriaChange = (e: any) => {
    setAuditInput((prev) => ({ ...prev, criteria: e.target.value as string[] }));
  };

  const handleObjectiveInputChange = (e: ChangeEvent<HTMLInputElement>) => {
  ...
  const isFormComplete = () =>
    !!newProgram.name && !!newProgram.startDate && !!newProgram.endDate && newProgram.audits.length > 0;
  ...
  const handleCreateProgram = async () => {
    ...
    try {
      await createAuditProgramWithAudits(newProgram, token);
      router.push("/audit/audit-program");
    } catch (error) {
      setErrors((prev) => ({
        ...prev,
        submit: "Failed to create program. Please try again.",
      }));
    }
  };
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <Tabs value={tab} onChange={handleTabChange} variant="fullWidth" className="mb-6 rounded-lg shadow-sm">
        <Tab
          label="Metadata"
          icon={newProgram.name && newProgram.startDate && newProgram.endDate ? <CheckCircleIcon color="success" /> : undefined}
          iconPosition="end"
        />
        <Tab
          label="Audits"
          icon={newProgram.audits.length > 0 ? <CheckCircleIcon color="success" /> : undefined}
          iconPosition="end"
        />
      </Tabs>
      ...
      >
        ...
        <Box className="flex justify-between mt-6">
          <Button
            variant="outlined"
            color="secondary"
            onClick={() => router.push("/audit/audit-program")}
          >
            Cancel
          </Button>
          <Button
...
// Optional: for Next.js App Router dynamic rendering
